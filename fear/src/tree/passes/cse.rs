use crate::tree::{Expr, FunctionDef, ValueId};
use crate::types::*;
...
fn rewrite(
    ...
    match expr {
        ...
        Expr::Call(func_id, params) => {
            let func = m.get_function(func_id).expect("cannot find function");
            (Expr::Call(func_id, params.clone()), func.signature.returns)
        }
        ...
        Expr::Load(volatile, ptr) => {
            let (ptr, ty_ptr) = rewrite(m, func, bid, *ptr, cnt, memo, new_values, changed);
            let expr = Expr::Load(volatile, Box::new(ptr));
            (expr, ty_ptr)
        }
        Expr::Store(volatile, ptr, value) => {
            let (ptr, ty_ptr) = rewrite(m, func, bid, *ptr, cnt, memo, new_values, changed);
            let (value, _) = rewrite(m, func, bid, *value, cnt, memo, new_values, changed);
            let expr = Expr::Store(volatile, Box::new(ptr), Box::new(value));
            (expr, ty_ptr)
        }
        Expr::PtrOffset(base, offset) => {
            let (base, ty_base) = rewrite(m, func, bid, *base, cnt, memo, new_values, changed);
            let (offset, _) = rewrite(m, func, bid, *offset, cnt, memo, new_values, changed);
        ...
        Expr::Add(a, b) => {
            let (a, ty_a) = rewrite(m, func, bid, *a, cnt, memo, new_values, changed);
            let (b, _) = rewrite(m, func, bid, *b, cnt, memo, new_values, changed);
