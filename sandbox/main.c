...
void foo_def(FearFunctionDef *def)
{
  ...
  FearValueId tmp0 = fearCreateAdd(def, entryBlock, FearInt8, const24, const42);
...
int main()
{
  ...
  if (fearLoweringHasCranelift())
  {
    ...
    FearModule *exm = fearBinaryFetchModuleFromFile(fileno(binmodf));
    fclose(binmodf);

    FILE *exf_obj = fopen("ex.o", "w");
    fearEmitCraneliftObjectToFile(m, FearOptFull, fileno(exf_obj));
    fclose(exf_obj);
  }
